...
df_model <- df %>% select(cluster = "No_non_DLS" , colonizer, lcapped, lpd1500s, lat_abst,
...
model_rule  <- multinom(cluster ~ ruleoflaw+colonizer, data = df_model,trace=TRUE)
model_set  <- multinom(cluster ~ colonizer+lcapped, data = df_model,trace=TRUE)
