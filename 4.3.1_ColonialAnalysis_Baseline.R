...
#Fit model------------------------------------------------------
df_model <- df %>%
  filter(colonizer!="Not colonized")%>% #Filter out countries that have not been colonized
  mutate(colonizer=factor(colonizer))
...
#Model all data
fit1 <- multinom(cluster ~ colonizer,
                data=df_model)
summary(fit1)           
...
fit2 <- multinom(cluster ~ colonizer+prienr1900,
                 data=df_model)
summary(fit2)           
